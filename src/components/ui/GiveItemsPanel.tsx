...
import { Gift, Plus, Trash2, ChevronDown, Coins } from "lucide-react";
...
export default function GiveItemsPanel({
  ...
  const handleGiveItems = async () => {
    if (!selectedUser) {
      setError("Pilih user terlebih dahulu");
      return;
    }

    if (selectedItems.length === 0 && eonixAmount === 0) {
      setError("Pilih minimal 1 item atau masukkan jumlah Eonix");
      return;
    }

    const itemsText = selectedItems.map(i => `${i.amount}x ${i.name}`).join(', ');
    const eonixText = eonixAmount > 0 ? `${eonixAmount} Eonix` : '';
    const giveText = [itemsText, eonixText].filter(Boolean).join(' + ');

    const confirmAction = window.confirm(
      `Berikan kepada ${selectedUser.name}:\n${giveText}`
    );
    ...
    try {
      const res = await onGiveItems(
        selectedUser.id,
        selectedItems.map(i => ({ id: i.id, type: i.type, amount: i.amount })),
        eonixAmount > 0 ? eonixAmount : undefined
      );
            ...
            prev.map(u =>
              u.id === selectedUser.id
                ? {
                  ...
                  rallyData: {
                    ...u.rallyData!,
                    enonix: (u.rallyData?.enonix || 0) + eonixAmount
                  }
                }
                : u
            )
          ...
          setSelectedUser(prev =>
            prev
              ? {
                ...
                rallyData: {
                  ...prev.rallyData!,
                  enonix: (prev.rallyData?.enonix || 0) + eonixAmount
                }
              }
              : null
          );
  ...
  return (
    <div className="w-full bg-black/40 backdrop-blur-md rounded-2xl border-3 border-[#684095] shadow-2xl p-6 mb-6">
      ...
      <div className="mb-6 relative" ref={dropdownRef}>
        <label className="text-[#78CCEE] font-bold mb-2 block uppercase text-xs tracking-widest">SELECT USER</label>
      ...
      {selectedUser && (
        <div className="mb-6 p-4 bg-[#3E344A]/60 rounded-lg border-2 border-[#78CCEE]">
          ...
          <p className="text-[#41FFA3] text-sm mt-1">Current Eonix: {selectedUser.rallyData?.enonix || 0}</p>
        </div>
      )}
      ...
      {selectedUser && (
        <div className="mb-6">
          <label className="text-[#78CCEE] font-bold mb-2 block uppercase text-xs tracking-widest flex items-center gap-2">
            <Coins size={16} /> GIVE EONIX
          </label>
          <input
            type="number"
            min="0"
            value={eonixAmount}
            onChange={(e) => setEonixAmount(Math.max(0, parseInt(e.target.value) || 0))}
            className="w-full px-4 py-2 bg-[#3E344A] border-2 border-[#684095] rounded-lg text-white focus:outline-none focus:border-[#78CCEE] transition-colors"
            placeholder="Enter Eonix amount..."
          />
        </div>
      )}
      ...
      {selectedUser && (
        <div className="mb-6">
          <label className="text-[#78CCEE] font-bold mb-2 block uppercase text-xs tracking-widest">
            SELECT ITEMS
          </label>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {/* Big Items */}
            <div>
              <p className="text-[#41FFA3] font-bold mb-2 text-xs uppercase">Big Items:</p>
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {bigItems.map((item) => (
                  <button
                    key={item.id}
                    onClick={() => addItem(item, 'big')}
                    disabled={selectedItems.some(i => i.id === item.id && i.type === 'big')}
                    className="w-full bg-[#3E344A] hover:bg-[#78CCEE]/20 disabled:opacity-50 disabled:cursor-not-allowed border-2 border-[#684095] hover:border-[#78CCEE] p-3 rounded-lg text-left transition-all"
                  >
            ...
            {/* Small Items */}
            <div>
              <p className="text-[#78CCEE] font-bold mb-2 text-xs uppercase">Small Items (Materials):</p>
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {smallItems.map((item) => (
                  <button
                    key={item.id}
                    onClick={() => addItem(item, 'small')}
                    disabled={selectedItems.some(i => i.id === item.id && i.type === 'small')}
                    className="w-full bg-[#3E344A] hover:bg-[#78CCEE]/20 disabled:opacity-50 disabled:cursor-not-allowed border-2 border-[#684095] hover:border-[#78CCEE] p-3 rounded-lg text-left transition-all"
                  >
                    <p className="text-white font-bold text-sm">{item.name}</p>
                  </button>
