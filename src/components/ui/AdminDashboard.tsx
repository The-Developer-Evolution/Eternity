...
export function AdminDashboard({ initialContestState, periods, activePeriodId }: AdminDashboardProps) {
  
  const { data: contestData, mutate } = useSWR<ContestResponse>(
    "/api/contest/status",
    fetcher,
    {
  ...
  const handleAction = async (action: string) => {
    ...
    try {
      switch (action) {
        case "start":
          await StartContestTimer(selectedPeriodId, duration);
          break;
        case "pause":
          await pauseContest();
          break;
  ...
  return (
    <div className="max-w-2xl mx-auto bg-gray-900/90 backdrop-blur-sm p-6 rounded-xl border border-[#684095] shadow-2xl mb-8">
      <div className="mb-6 text-center border-b border-gray-700 pb-4">
        <p className="text-gray-400 text-sm uppercase tracking-widest mb-1">Current Status</p>
      ...
      {(status === RallyPeriodStatus.NOT_STARTED || status === RallyPeriodStatus.ENDED) && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6 bg-gray-800/50 p-4 rounded-lg">
          <div className="flex flex-col gap-2">
            <label className="text-[#75E8F0] font-bold text-sm">SELECT EVENT PERIOD</label>
            <select
