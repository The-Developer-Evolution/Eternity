...
export default function CurrencyConvertInterface() {
  ...
  const [fromCurrency, setFromCurrency] = useState<CurrencyType>("IDR");
  ...
  const handleConvert = async () => {
    ...
    try {
      const result = await convertCurrency(selectedUser.id, amount, fromCurrency, toCurrency);
  ...
  const toggleCurrency = () => {
    setFromCurrency(prev => prev === "IDR" ? "USD" : "IDR");
  };

  const predictedResult = Math.floor(amount * exchangeRate);
    ...
    <div className="relative z-10 w-full max-w-4xl grid grid-cols-1 md:grid-cols-2 gap-8 p-4">
      ...
      <div className="bg-gray-900/80 backdrop-blur-md border border-[#AE00DE] p-6 rounded-xl flex flex-col gap-6 shadow-2xl">
        ...
        <div className="flex flex-col gap-2">
          <label className="text-gray-400 text-sm font-bold flex items-center gap-2">
            <Wallet size={16} /> CONVERSION PATH
          </label>
          
          <div className="flex items-center justify-between bg-gray-800 rounded-lg p-2 border border-gray-700">
            <div className={`flex-1 text-center py-3 rounded font-bold transition-all ${fromCurrency === "IDR" ? "bg-[#AE00DE]/20 text-[#AE00DE]" : "text-gray-500"}`}>
              IDR
            </div>
            
            <button 
              onClick={toggleCurrency}
              className="bg-gray-700 p-2 rounded-full hover:bg-gray-600 transition-colors mx-2 text-[#75E8F0]"
            >
              <ArrowRightLeft size={20} />
            </button>

            <div className={`flex-1 text-center py-3 rounded font-bold transition-all ${fromCurrency === "USD" ? "bg-[#AE00DE]/20 text-[#AE00DE]" : "text-gray-500"}`}>
              USD
            </div>
          </div>
        ...
        <div className="flex flex-col gap-4">
            <div className="flex justify-between items-center bg-gray-800/50 p-4 rounded">
             ...
             <div className="flex justify-between items-center bg-gray-800/50 p-4 rounded">
                ...
                <span className="font-bold text-lg text-red-400">- {amount.toLocaleString()} {fromCurrency}</span>
            </div>
            ...
            <div className="flex justify-center text-gray-500">
             ...
             <div className="flex justify-between items-center bg-gray-800/80 p-4 rounded border border-[#AE00DE]/30">
                <span className="text-gray-400 text-sm">RECEIVED (EST.)</span>
                <span className="font-bold text-2xl text-green-400">+ {predictedResult.toLocaleString()} {toCurrency}</span>
            </div>
        </div>
        
        <div className="mt-8 text-xs text-gray-500 text-center">
            Rates are fixed by the system.
        </div>
      </div>
    </div>
  );
